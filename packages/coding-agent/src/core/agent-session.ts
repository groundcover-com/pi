...
export class AgentSession {
	...
	private _handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (event.type === "message_end") {
			...
			if (event.message.role === "custom") {
				...
				await this.sessionManager.appendMessageAsync(event.message);
			}
	...
	async setModel(model: Model<any>): Promise<void> {
		...
		await this.sessionManager.appendModelChangeAsync(model.provider, model.id);
	...
	private async _cycleScopedModel(direction: "forward" | "backward"): Promise<ModelCycleResult | undefined> {
		...
		await this.sessionManager.appendModelChangeAsync(next.model.provider, next.model.id);
	...
	private async _cycleAvailableModel(direction: "forward" | "backward"): Promise<ModelCycleResult | undefined> {
		...
		await this.sessionManager.appendModelChangeAsync(nextModel.provider, nextModel.id);
	...
	async compact(customInstructions?: string): Promise<CompactionResult> {
		...
		try {
			...
			await this.sessionManager.appendCompactionAsync(
				summary,
				firstKeptEntryId,
				tokensBefore,
				details,
	...
	async recordBashResult(
		...
		if (this.isStreaming) {
			...
			// Save to session
			await this.sessionManager.appendMessageAsync(bashMessage);
		}
	}
